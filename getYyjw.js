...
function getChapter(chapter){
	...
	http.get('http://m.zongheng.com/h5/ajax/chapter?'+params,(res)=>{
		...
		res.on('end', () => {
			...
			//规范内容，去除冗余
			cleanContent = rawData.result.content.replace(/(（本章未完，请翻页）)|(（本章完）|(\<p\>)|(\<\/p\>))/g,function(macth,p1,p2,p3,p4){
				if (p1) return '';
				if (p2) return '';
				if (p3) return '';
				if (p4) return '\r\n';
			...
			// 写入文件
			fs.writeFile('yyjw.txt', oldText+cleanTitle+cleanContent, 'utf8', res => {
